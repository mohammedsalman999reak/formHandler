...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // Set logger level
      logger.setLogLevel(
        env.LOG_LEVEL || (NODE_ENV === "development" ? "DEBUG" : "WARN")
      );
      ...
      if (env.RATE_LIMITER) {
        ...
        if (!rateLimitResult.allowed) {
          ...
          return ResponseHelper.createRateLimitError(
            rateLimitResult,
            corsHeaders
          );
        }
      ...
      // 2. Bot Detection
      if (
        env.ENABLE_BOT_DETECTION === "true" &&
        securityChecks.isBotRequest(request)
      ) {
      ...
      if (request.method !== "GET" && env.ENABLE_CSRF_PROTECTION === "true") {
        const securityHelper = (await import("./utils/security.js"))
          .SecurityHelper;
      ...
      if (env.ALLOWED_ORIGINS && env.ALLOWED_ORIGINS !== "*") {
        const originValid = authHandler.validateOrigin(
          request,
          env.ALLOWED_ORIGINS
        );
        if (!originValid) {
      ...
      if (env.TURNSTILE_SECRET_KEY) {
        ...
        const validTurnstile = await security.validateTurnstileToken(
          turnstileToken,
          clientIP,
          env
        );
      ...
      // Process file upload
      const fileInfo = await this.processFileUpload(
        rawData,
        ALLOWED_FILE_TYPES,
        MAX_FILE_SIZE,
        logger
      ...
      const [airtableResult, emailResult] = await Promise.allSettled([
        ...
        emailService.sendNotification(
          { ...submission, file: fileInfo.data },
          env
        ),
      ]);

      // Log operation results
      this.logOperationResults(
        airtableResult,
        emailResult,
        logger,
  ...
  async processFileUpload(rawData, allowedTypes, maxSize, logger) {
    try {
      ...
      const { base64Data, fileType } = FileProcessor.processFileData(
        fileData,
        rawData.file,
        fileName
      );
  ...
  logOperationResults(airtableResult, emailResult, logger, submissionId) {
    const airtableSuccess =
      airtableResult.status === "fulfilled" && airtableResult.value.success;
    const emailSuccess =
    ...
    logger.log("Form submission operations completed", {
      ...
      airtable: {
        ...
        error: airtableSuccess
          ? null
          : airtableResult.value?.error || airtableResult.reason,
      },
    ...
    if (!airtableSuccess) {
      console.log(
        "AIRTABLE ERROR DETAILED:",
        JSON.stringify(airtableResult.value || airtableResult.reason)
      );
    }
